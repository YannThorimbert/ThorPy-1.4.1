"""Provide functions and a class to store and order elements of an element"""
...
def v_store(father, children, xcoord=None, ystart="auto", margin=None,
    ...
    (xc, yc) = father.get_storer_center()
    (x0, y0) = father.get_storer_topleft()
...
def h_store(father, children, ycoord=None, xstart="auto", margin=None, gap=None,
             align="center"):
    children = [c for c in children if not c._bartarget]
    children = sorted(children, key=lambda x: x.rank)
...
def store(father, elements=None, mode="v", x="auto", y="auto", margin=None,
            gap=None, align="center"):
    if elements is None:
        elements = father.get_elements()
    if mode == "v" or mode == "vertical":
        x = None if x == "auto" else x
        size = v_store(father, elements, x, y, margin, gap, align)
    elif mode == "h" or mode == "horizontal":
        y = None if y == "auto" else y
        size = h_store(father, elements, y, x, margin, gap, align)
    else:
...
class Storer:
    ...
    def center(self):
        ...
        size=store(father=self._element,
