...
def get_column_from_array(array, column, typ=float):
    l = list()
##    print len(array), len(array[0]), column
    for line in array:
        if line:
            value = typ(line[column])
            l.append(value)
    return l

def get_column_from_datafile(filename, column, sep=" ", typ=float):
    array = get_data_as_2d_array(filename, sep)
    return get_column_from_array(array, column, typ)
...
class ParamsLoader(object):
    ...
    def get(self, param, typ=str, typ2=int):
